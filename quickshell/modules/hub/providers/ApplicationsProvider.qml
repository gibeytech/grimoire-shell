...
QtObject {
    ...
    function execute(command) {
        const actionCommand = String(command || "")

        if (!actionCommand.startsWith("desktop:")) {
            return false
        }

        const desktopId = actionCommand.replace("desktop:", "")
        const entry = DesktopEntries.byId(desktopId)

        if (!entry) {
            console.warn("[ApplicationsProvider] Desktop entry not found:", desktopId)
            return false
        }
